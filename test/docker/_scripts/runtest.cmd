...
docker run -v %_MOUNTPATH%:/tests --rm %_IMAGENAME%:latest bash /tests/%_TESTNAME%.sh %_PACKAGENAME% > %_OUTFILE%

IF EXIST %_MOUNTPATH%\%_TESTNAME%.reference.txt ( 
    comp /M %_MOUNTPATH%\%_TESTNAME%.reference.txt %_OUTFILE% > NULL
    if errorlevel 1 (
        type %_OUTFILE%
        set _RESULT=1
    ) else (
